...
class ThreeTenNetwork {
	/**
	 * sets up Dijkstras' whit given parameters.
	 * @param graph representation.
	 * @param startNode node.
	 * @param queue holds nodes.
	 * @param hostToDijkstraNode node.
	 * */
	public static void setupDijkstras(Network graph, Host startNode, PriorityQueue<DijkstraNode> queue, HashTable<Host,DijkstraNode> hostToDijkstraNode) {
		...
		//So you began by getting all nodes i.e vertecies in graph then convert them into dijkstranodes 
		// then add them to queue(this takes in only type Dijkstranode)
		//after raping the nodes we need to set distance starting with startNode with distance 0;
		//then you also add them to hashTable<host,DijkstraNode>
		if(graph.containsVertex(startNode)) {
			DijkstraNode dnode= new DijkstraNode(startNode);
			dnode.distance=0;//set the distance

			queue.add(dnode);//add to queue
			hostToDijkstraNode.add(startNode, dnode);//add to hastable to look for it again
			for(Host e: graph.getVertices()) {//going through graph and getting the nodes/vertices/hosts
				if(e.equals(startNode)) {
					continue;
				}else {
					DijkstraNode dnod= new DijkstraNode(e);
					queue.add(dnod);
					hostToDijkstraNode.add(e, dnod);
				}
	...
	/**
	 * set hashTable for given startNode from given graph.
	 * @param graph nodes.
	 * @param startNode node position.
	 * @param hostToDijkstraNode node.
	 * */
	public static void setupTables(Network graph, Host startNode, HashTable<Host,DijkstraNode> hostToDijkstraNode) {
		...
		//For each host(vertex) in the graph you'll need to:
		//1. Get the DijkstraNode representation (which comes from the hostToDijkstraNode)
		//2. Discover the next hop from the start node towards the other node
		//3. Add the pair (other host(node), next hop) to the routing table for the start node
		
		if(graph.containsVertex(startNode)){//checking if this exists 
			RoutingTable rt= startNode.getRoutingTable();
			for(Host v: graph.getVertices()) {// so 'other host' is v , 'next hop'is host
				DijkstraNode dnode= hostToDijkstraNode.get(v);
				//ok so you have to create table to startnode
				//(dest, nextnode)
				if(v.equals(startNode)) {
					//making sure where are not counting the startnode
					continue;
				}else {
					if(dnode.parent!=null) {
						while(dnode.parent.host!=startNode) {
							dnode=dnode.parent;//go up to this parent 
						}
						rt.add(v, dnode.host);//adding to the startNode routing table
					}
		...
		//Hint: To discover the next hop(i.e next node that is in path) from the start node to
		// the destination(other node). Remember that Dijkstra's algorithm
